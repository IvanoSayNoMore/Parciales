	...
	if(arrayPersonPoint != NULL && lenPointer > 0)
	{
		...
		if(auxPointeBussy != RETORNONEGATIVO)
		{
					...
					,MINIMOIDS, MAXIMOSIDS, 2)==RETORNOPOSITIVO)
			{
				...
				/*if(person_findOnePersonById(arrayPersonPoint, auxId, &auxPointeBussy, lenPointer)==RETORNOPOSITIVO)
				{
					if(person_deletIndexArray(arrayPersonPoint, auxPointeBussy)==RETORNOPOSITIVO)
					{
						retorno=RETORNOPOSITIVO;
					}
				}
